...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            # 跳过 meta device 上的张量（init_empty_weights 时）
            if module.weight.device.type == "meta":
                return
            # 检查是否是方阵（ttt_proj 是 hidden_size x hidden_size）
            if module.weight.shape[0] == module.weight.shape[1]:
                ...
                if hasattr(weight_data, '_local_tensor'):
                    ...
                    print(f"Init Linear DTensor (square): {module.weight.shape}, local_shape={local_tensor.shape}, rank={rank}")
                else:
                    # 普通张量
                    weight_data.zero_()
                    ...
                    print(f"Init Linear (square): {module.weight.shape}, diag mean={weight_data.diagonal().mean().item():.6f}")
            else:
                # 非方阵 Linear 只初始化为零（continual pretrain 时这些已有权重）
                print(f"Init Linear (non-square): {module.weight.shape}")
