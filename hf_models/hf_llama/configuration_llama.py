...
class LlamaConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # OPD-TTT settings
        # opdttt_layers: 如果为 None，默认使用 ttt_layers
        self.opdttt_mode = opdttt_mode
        self.opdttt_layers = opdttt_layers if opdttt_layers is not None else ttt_layers
        self.lambda_ntp = lambda_ntp
        self.lambda_align_rep = lambda_align_rep
        self.lambda_kl = lambda_kl
        self.lambda_lm = lambda_lm
        # weight_adaptation: 'fixed' 或 'adaptive'
        # - 'fixed': 使用固定权重
        # - 'adaptive': 根据 NTP 和教师梯度的余弦相似度动态调整权重
        if weight_adaptation not in {"fixed", "adaptive"}:
            raise ValueError("weight_adaptation must be one of {'fixed', 'adaptive'}")
        self.weight_adaptation = weight_adaptation
        # teacher_proj_init: 'random' 或 'pca'
        # - 'random': 随机初始化（默认）
        # - 'pca': 使用教师嵌入的 PCA 初始化
        if teacher_proj_init not in {"random", "pca", "pca_tied"}:
            raise ValueError("teacher_proj_init must be one of {'random', 'pca', 'pca_tied'}")
        self.teacher_proj_init = teacher_proj_init
        # teacher_embeddings_for_init: 用于 PCA 初始化的教师嵌入
        # 形状应为 [vocab_size, hidden_size] 或 [num_samples, hidden_size]
        self.teacher_embeddings_for_init = teacher_embeddings_for_init
