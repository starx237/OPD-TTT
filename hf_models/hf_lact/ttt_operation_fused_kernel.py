...
# Multi-level import fallback for better compatibility
try:
    # Try absolute import with full module path
    from hf_models.hf_lact.lact_triton_kernels.lact_swiglu_ffn import fused_swiglu_ffn_fwd
    from hf_models.hf_lact.lact_triton_kernels.lact_fw_grad import (
        fused_lact_swiglu_ffn_fast_weight_grads,
    )
    from hf_models.hf_lact.lact_triton_kernels.triton_prenorm_update_with_momentum import (
        fused_prenorm_update_with_momentum_and_l2_norm,
    )
    from hf_models.hf_lact.lact_triton_kernels.l2norm_triton_kernels import l2_norm_add_fused
except ImportError:
    try:
        # Try relative import
