...
# Compatibility: triton_op and wrap_triton are only available in PyTorch 2.5+
try:
    from torch.library import triton_op, wrap_triton
    HAS_TRITON_OP = True
except ImportError:
    HAS_TRITON_OP = False
    # Use custom_op as fallback for PyTorch 2.4
    try:
...
### Warpping around with triton_op (compatibility wrapper for PyTorch 2.4)
if HAS_TRITON_OP:
    ...
    def swiglu_backward_three_bmm_triton_op(
        ...
        return _swiglu_backward_three_bmm_triton_impl(W0_W2, W1, X, V)
else:
    # For PyTorch 2.4, use the function directly without triton_op wrapper
    # The function will still work, just without the torch.library optimization
    swiglu_backward_three_bmm_triton_op = swiglu_backward_three_bmm_triton


def _swiglu_backward_three_bmm_triton_impl(
    W0_W2: torch.Tensor, W1: torch.Tensor, X: torch.Tensor, V: torch.Tensor
) -> Tuple[torch.Tensor, torch.Tensor]:
    B, M_times_2, K = W0_W2.shape
    M = M_times_2 // 2
