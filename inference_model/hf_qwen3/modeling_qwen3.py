...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x, t=None, past_w=None):  # TTT: added t, past_w params
        h = self.act_fn(self.gate_proj(x)) * self.up_proj(x)
        # TTT: branch on whether this is a TTT layer
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx  # TTT: store layer_idx
        ...
        self.ttt_chunk = getattr(config, "ttt_chunk", 8192)  # TTT
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if target_states is None and hasattr(self.mlp, "ttt_conv"):
...
class Qwen3PreTrainedModel(PreTrainedModel):
    ...
    # TTT: adds Conv1d handling for TTT components; rest identical to HF library
    def _init_weights(self, module):
        ...
        elif isinstance(module, nn.Conv1d):  # TTT: zero-init for ttt_conv
...
class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        self.ttt_layers = getattr(config, "ttt_layers", [])  # TTT
        self.ttt_mode = getattr(config, "ttt_mode", False)  # TTT
    ...
    def _resolve_ttt_target_states(
        ...
        if not self.ttt_mode or decoder_layer.layer_idx not in self.ttt_layers:
            return None
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache and past_key_values is None:
            past_key_values = TTTDynamicCache(config=self.config) if self.ttt_mode else DynamicCache(config=self.config)  # TTT
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    # TTT: ensure TTT-aware cache and bs=1 constraint
