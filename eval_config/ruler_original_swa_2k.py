...
with read_base():
    ...
    from .models_original_swa import models as qwen3_models
    from opencompass.configs.summarizers.groups.ruler import ruler_summary_groups

import_datasets = sum([qa_datasets, niah_datasets, vt_datasets, fwe_datasets, cwe_datasets], [])

NUM_SAMPLES = 100
max_seq_lens = [2048]
abbr_suffixs = ["2k"]

work_dir = f"./results/ruler_original_swa/2k"

model_settings = [[qwen3_models[i], qwen3_models[i]["path"]] for i in range(len(qwen3_models))]
...
infer = dict(
    ...
    runner=dict(type=LocalRunner, max_num_workers=16, task=dict(type=OpenICLInferTask), retry=5),
)
