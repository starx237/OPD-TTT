...
def build_optimizer(
    ...
    Backward compatibility: optimizer_type can be either a string (legacy) or
    an OptimizerConfig object (new). When OptimizerConfig is provided, its values
    override the corresponding function parameters.
    """
    # Handle OptimizerConfig for backward compatibility
    from ..arguments.arguments_types import OptimizerConfig
    if isinstance(optimizer_type, OptimizerConfig):
        optimizer_config = optimizer_type
        optimizer_type = optimizer_config.type
        # Override function parameters with OptimizerConfig values if provided
        if optimizer_config.lr is not None and lr == 1e-3:  # Only override if default
            lr = optimizer_config.lr
        if optimizer_config.weight_decay is not None and weight_decay == 1e-2:  # Only override if default
            weight_decay = optimizer_config.weight_decay
        if optimizer_config.max_grad_norm is not None:
            # Note: max_grad_norm is handled by the trainer, not here
            pass
        if optimizer_config.no_decay_modules is not None and no_decay_modules is None:
            no_decay_modules = optimizer_config.no_decay_modules
        if optimizer_config.no_decay_params is not None and no_decay_params is None:
            no_decay_params = optimizer_config.no_decay_params
