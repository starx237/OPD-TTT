...
# Disable expandable_segments to avoid INTERNAL ASSERT FAILED errors
torch_version = tuple(int(x) for x in torch.__version__.split(".")[:2])
if (2, 4) <= torch_version < (2, 5):
    if "PYTORCH_CUDA_ALLOC_CONF" not in os.environ:
        os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:False"
    elif "expandable_segments" not in os.environ["PYTORCH_CUDA_ALLOC_CONF"]:
        os.environ["PYTORCH_CUDA_ALLOC_CONF"] += ",expandable_segments:False"

# Compatibility: CPUOffloadPolicy renamed to CPUOffload in PyTorch 2.5+
try:
...
def parallelize_model_fsdp2(
    ...
    **kwargs,
    ...
    if weights_path is None:
        ...
        # Compatibility: distribute_tensor moved in different PyTorch versions
        try:
...
def build_parallelize_model(
    ...
    **kwargs,
    ...
    if enable_gradient_checkpointing and hasattr(model, "gradient_checkpointing_enable"):
        ...
        # Compatibility: PyTorch 2.4.x requires reentrant mode with FSDP2 to avoid
        # expandable_segment allocator issues. Auto-detect and force reentrant mode.
        torch_version = tuple(int(x) for x in torch.__version__.split(".")[:2])
        if parallel_state.fsdp_enabled and parallel_state.dp_mode == "fsdp2":
            if (2, 4) <= torch_version < (2, 5):
                if not use_reentrant:
                    logger.warning_rank0(
                        f"PyTorch {torch.__version__} with FSDP2 requires reentrant gradient checkpointing "
                        "to avoid expandable_segment allocator issues. Forcing use_reentrant=True."
                    )
                    use_reentrant = True

        # Compatibility: DDP requires reentrant mode with gradient checkpointing to avoid
        # "Expected to mark a variable ready only once" errors. Non-reentrant mode is not
        # compatible with DDP's parameter marking mechanism.
        if parallel_state.fsdp_enabled and parallel_state.dp_mode == "ddp":
            if not use_reentrant:
                logger.warning_rank0(
                    f"DDP mode requires reentrant gradient checkpointing to avoid "
                    "'Expected to mark a variable ready only once' errors. Forcing use_reentrant=True."
                )
                use_reentrant = True
