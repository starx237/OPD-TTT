...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, preserve_rng_state, *args):
        ...
        if isinstance(ctx.run_function, torch.nn.Module):
            ctx.patch_module = ctx.run_function
        elif hasattr(ctx.run_function, "__self__") and isinstance(ctx.run_function.__self__, torch.nn.Module):
            ctx.patch_module = ctx.run_function.__self__
        else:
            # 针对 PyTorch 2.4+ 纯函数包装（如 torch._compile / checkpoint 封装）的兼容
            ctx.patch_module = None
            
            # 尝试从函数的闭包（closure）中挖出原本的 nn.Module 实例
            if hasattr(ctx.run_function, "__closure__") and ctx.run_function.__closure__ is not None:
                for cell in ctx.run_function.__closure__:
                    if isinstance(cell.cell_contents, torch.nn.Module):
                        ctx.patch_module = cell.cell_contents
                        break
