...
class TrainingArguments:
    ...
    # Flat optimizer parameters for backward compatibility with legacy training scripts
    # These are synchronized to optimizer in __post_init__
    lr: Optional[float] = field(
        default=None,
        metadata={"help": "Learning rate (flat alias for optimizer.lr)."},
    )
    lr_min: Optional[float] = field(
        default=None,
        metadata={"help": "Minimum learning rate (flat alias for optimizer.lr_min)."},
    )
    lr_start: Optional[float] = field(
        default=None,
        metadata={"help": "Learning rate for warmup start (flat alias for optimizer.lr_start)."},
    )
    lr_warmup_ratio: Optional[float] = field(
        default=None,
        metadata={"help": "Ratio of learning rate warmup steps (flat alias for optimizer.lr_warmup_ratio)."},
    )
    lr_decay_style: Optional[str] = field(
        default=None,
        metadata={"help": "Learning rate decay style (flat alias for optimizer.lr_decay_style)."},
    )
    lr_decay_ratio: Optional[float] = field(
        default=None,
        metadata={"help": "Ratio of learning rate decay steps (flat alias for optimizer.lr_decay_ratio)."},
    )
    weight_decay: Optional[float] = field(
        default=None,
        metadata={"help": "L2 regularization strength (flat alias for optimizer.weight_decay)."},
    )
    max_grad_norm: Optional[float] = field(
        default=None,
        metadata={"help": "Clip value for gradient norm (flat alias for optimizer.max_grad_norm)."},
    )
    ...
    def __post_init__(self):
        # Handle legacy flat optimizer parameter for backward compatibility
        # If optimizer is a string (legacy format), convert it to OptimizerConfig
        if isinstance(self.optimizer, str):
            from .arguments_types import OptimizerConfig
            optimizer_type = self.optimizer
            self.optimizer = OptimizerConfig(type=optimizer_type)

        # Sync flat optimizer parameters to optimizer config (backward compatibility)
        if self.lr is not None:
            self.optimizer.lr = self.lr
        if self.lr_min is not None:
            self.optimizer.lr_min = self.lr_min
        if self.lr_start is not None:
            self.optimizer.lr_start = self.lr_start
        if self.lr_warmup_ratio is not None:
            self.optimizer.lr_warmup_ratio = self.lr_warmup_ratio
        if self.lr_decay_style is not None:
            self.optimizer.lr_decay_style = self.lr_decay_style
        if self.lr_decay_ratio is not None:
            self.optimizer.lr_decay_ratio = self.lr_decay_ratio
    ...
    def _add_backward_compat_aliases(self):
        """Add backward compatibility aliases for legacy training code."""
        # checkpoint-related aliases
        self.output_dir = self.checkpoint.output_dir
        self.ckpt_manager = self.checkpoint.manager
        self.save_checkpoint_path = self.checkpoint.save_path
        self.save_steps = self.checkpoint.save_steps
        self.save_epochs = self.checkpoint.save_epochs
        self.save_hf_weights = self.checkpoint.save_hf_weights
        self.load_checkpoint_path = self.checkpoint.load_path
        self.model_assets_dir = self.checkpoint.model_assets_dir

        # accelerator-related aliases
        self.data_parallel_size = self.accelerator.dp_size
        self.data_parallel_replicate_size = self.accelerator.dp_replicate_size
        self.data_parallel_shard_size = self.accelerator.dp_shard_size
        self.tensor_parallel_size = self.accelerator.tp_size
        self.expert_parallel_size = self.accelerator.ep_size
        self.pipeline_parallel_size = self.accelerator.pp_size
        self.context_parallel_size = self.accelerator.cp_size
        self.ulysses_parallel_size = self.accelerator.ulysses_size
        self.data_parallel_mode = self.accelerator.fsdp_config.fsdp_mode
        self.enable_full_shard = (self.accelerator.fsdp_config.fsdp_mode == "fsdp2")
        self.enable_mixed_precision = self.accelerator.fsdp_config.mixed_precision.enable
        self.enable_gradient_checkpointing = self.gradient_checkpointing.enable
        self.enable_fsdp_offload = self.accelerator.fsdp_config.offload
        self.enable_reentrant = self.gradient_checkpointing.enable_reentrant
        self.enable_forward_prefetch = self.accelerator.fsdp_config.forward_prefetch
        self.enable_activation_offload = self.accelerator.offload_config.enable_activation
        self.activation_gpu_limit = self.accelerator.offload_config.activation_gpu_limit

        # optimizer-related aliases
        self.lr = self.optimizer.lr
        self.lr_min = self.optimizer.lr_min
        self.lr_start = self.optimizer.lr_start
        self.lr_decay_style = self.optimizer.lr_decay_style
        self.lr_decay_ratio = self.optimizer.lr_decay_ratio
        self.lr_warmup_ratio = self.optimizer.lr_warmup_ratio
        self.weight_decay = self.optimizer.weight_decay
        self.max_grad_norm = self.optimizer.max_grad_norm

        # wandb-related aliases
        self.use_wandb = self.wandb.enable
        self.wandb_project = self.wandb.project
        self.wandb_name = self.wandb.name

        # profile-related aliases
        self.profile_this_rank = self.profile.enable
        self.profile_start_step = self.profile.start_step
        self.profile_end_step = self.profile.end_step
        self.profile_trace_dir = self.profile.trace_dir
        self.profile_record_shapes = self.profile.record_shapes
        self.profile_profile_memory = self.profile.profile_memory
        self.profile_with_stack = self.profile.with_stack


# ================================ Backward Compatibility Properties ========================
# The following properties provide backward compatibility for legacy training code
# that expects flat attribute names. They map the new nested structure to legacy names.
# These properties are added dynamically to avoid cluttering the dataclass definition.


def _add_backward_compat_properties(cls):
    """Add backward compatibility properties to a dataclass."""

    # TrainingArguments properties
    if cls.__name__ == "TrainingArguments":

        @property
        def output_dir(self):
            return self.checkpoint.output_dir

        @property
        def ckpt_manager(self):
            return self.checkpoint.manager

        @property
        def save_checkpoint_path(self):
            return self.checkpoint.save_path

        @property
        def save_steps(self):
            return self.checkpoint.save_steps

        @property
        def save_epochs(self):
            return self.checkpoint.save_epochs

        @property
        def save_hf_weights(self):
            return self.checkpoint.save_hf_weights

        @property
        def load_checkpoint_path(self):
            return self.checkpoint.load_path

        @property
        def model_assets_dir(self):
            return self.checkpoint.model_assets_dir

        @property
        def data_parallel_size(self):
            return self.accelerator.dp_size

        @property
        def data_parallel_replicate_size(self):
            return self.accelerator.dp_replicate_size

        @property
        def data_parallel_shard_size(self):
            return self.accelerator.dp_shard_size

        @property
        def tensor_parallel_size(self):
            return self.accelerator.tp_size

        @property
        def expert_parallel_size(self):
            return self.accelerator.ep_size

        @property
        def pipeline_parallel_size(self):
            return self.accelerator.pp_size

        @property
        def context_parallel_size(self):
            return self.accelerator.cp_size

        @property
        def ulysses_parallel_size(self):
            return self.accelerator.ulysses_size

        @property
        def data_parallel_mode(self):
            return self.accelerator.fsdp_config.fsdp_mode

        @property
        def enable_full_shard(self):
            return self.accelerator.fsdp_config.fsdp_mode == "fsdp2"

        @property
        def enable_mixed_precision(self):
            return self.accelerator.fsdp_config.mixed_precision.enable

        @property
        def enable_gradient_checkpointing(self):
            return self.gradient_checkpointing.enable

        @property
        def enable_fsdp_offload(self):
            return self.accelerator.fsdp_config.offload

        @property
        def enable_reentrant(self):
            return self.gradient_checkpointing.enable_reentrant

        @property
        def enable_forward_prefetch(self):
            return self.accelerator.fsdp_config.forward_prefetch

        @property
        def enable_activation_offload(self):
            return self.accelerator.offload_config.enable_activation

        @property
        def activation_gpu_limit(self):
            return self.accelerator.offload_config.activation_gpu_limit

        @property
        def use_wandb(self):
            return self.wandb.enable
        ...
        @property
        def profile_this_rank(self):
            return self.profile.enable

        @property
        def profile_start_step(self):
            return self.profile.start_step

        @property
        def profile_end_step(self):
            return self.profile.end_step

        @property
        def profile_trace_dir(self):
            return self.profile.trace_dir

        @property
        def profile_record_shapes(self):
            return self.profile.record_shapes

        @property
        def profile_profile_memory(self):
            return self.profile.profile_memory

        @property
        def profile_with_stack(self):
            return self.profile.with_stack
        ...
        @property
        def rmpad_with_pos_ids(self):
            return False

        @property
        def dyn_bsz_margin(self):
            return 0

        @property
        def dyn_bsz_buffer_size(self):
            return 500

        # Add all properties to the class
        cls.output_dir = output_dir
        cls.ckpt_manager = ckpt_manager
        cls.save_checkpoint_path = save_checkpoint_path
        cls.save_steps = save_steps
        cls.save_epochs = save_epochs
        cls.save_hf_weights = save_hf_weights
        cls.load_checkpoint_path = load_checkpoint_path
        cls.model_assets_dir = model_assets_dir
        cls.data_parallel_size = data_parallel_size
        cls.data_parallel_replicate_size = data_parallel_replicate_size
        cls.data_parallel_shard_size = data_parallel_shard_size
        cls.tensor_parallel_size = tensor_parallel_size
        cls.expert_parallel_size = expert_parallel_size
        cls.pipeline_parallel_size = pipeline_parallel_size
        cls.context_parallel_size = context_parallel_size
        cls.ulysses_parallel_size = ulysses_parallel_size
        cls.data_parallel_mode = data_parallel_mode
        cls.enable_full_shard = enable_full_shard
        cls.enable_mixed_precision = enable_mixed_precision
        cls.enable_gradient_checkpointing = enable_gradient_checkpointing
        cls.enable_fsdp_offload = enable_fsdp_offload
        cls.enable_reentrant = enable_reentrant
        cls.enable_forward_prefetch = enable_forward_prefetch
        cls.enable_activation_offload = enable_activation_offload
        cls.activation_gpu_limit = activation_gpu_limit
        cls.use_wandb = use_wandb
        cls.wandb_project = wandb_project
        cls.wandb_name = wandb_name
        cls.profile_this_rank = profile_this_rank
        cls.profile_start_step = profile_start_step
        cls.profile_end_step = profile_end_step
        cls.profile_trace_dir = profile_trace_dir
        cls.profile_record_shapes = profile_record_shapes
        cls.profile_profile_memory = profile_profile_memory
        cls.profile_with_stack = profile_with_stack
        cls.rmpad = rmpad
        cls.rmpad_with_pos_ids = rmpad_with_pos_ids
        cls.dyn_bsz_margin = dyn_bsz_margin
        cls.dyn_bsz_buffer_size = dyn_bsz_buffer_size

    # DataArguments properties
    if cls.__name__ == "DataArguments":
        ...
        cls.drop_last = drop_last
        cls.pin_memory = pin_memory
        cls.prefetch_factor = prefetch_factor
...
class ModelArguments:
    ...
    # Flat parameter names for backward compatibility
    # These are synchronized with ops_implementation in __post_init__
    attn_implementation: Optional[str] = field(
        default=None,
        metadata={
            "help": "Attention implementation (flat alias for ops_implementation.attn_implementation). "
            "Options: eager, sdpa, flash_attention_2, flash_attention_3, flash_attention_4, native-sparse."
        },
    )
    moe_implementation: Optional[str] = field(
        default=None,
        metadata={
            "help": "MoE experts forward (flat alias for ops_implementation.moe_implementation). "
            "Options: eager, fused_triton, fused_quack, fused_npu."
        },
    )
    ops_implementation: OpsImplementationConfig = field(default_factory=OpsImplementationConfig)

    def __post_init__(self):
        # Sync flat parameter names to ops_implementation (backward compatibility)
...
class DataArguments:
    ...
    data_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Base directory for data paths. Used to resolve relative train_path."},
    )
    dataloader_type: str = field(
        default="native",
        metadata={"help": "Type of the dataloader. Legacy field; maps to dataloader.type."},
    )
...
# Apply backward compatibility properties to argument classes (must be at end of file after class definitions)


TrainingArguments = _add_backward_compat_properties(TrainingArguments)
ModelArguments = _add_backward_compat_properties(ModelArguments)
DataArguments = _add_backward_compat_properties(DataArguments)
