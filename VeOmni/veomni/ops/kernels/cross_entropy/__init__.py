...
def ForCausalLMLoss(
    ...
    **kwargs,
    ...
    # NOTE: Skip float conversion for FSDP2 to avoid OOM with large sequences.
    # bfloat16 precision is generally sufficient for training.
    if logits is not None and cross_entropy_fn.__name__ != "eager_cross_entropy":
        # Only convert to float for non-eager implementations that may need it
        # For eager mode (transformers default), keep original dtype to save memory
        try:
    ...
    # HF's original code does: loss = self.loss_function(logits=logits, labels=labels, ...)
    # which doesn't unpack the tuple, causing model_outputs.loss to be a tuple.
