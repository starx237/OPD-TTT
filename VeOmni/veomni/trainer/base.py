...
class BaseTrainer(Stateful, ABC):
    ...
    def _build_parallelized_model(self):
        ...
        # === 兼容性处理：安全提取梯度检查点配置 ===
        gcp_config = getattr(args.train, "gradient_checkpointing", None)
        
        # 兼容新旧格式的 enable
        enable_gcp = (
            gcp_config.enable if (gcp_config and hasattr(gcp_config, "enable"))
            else getattr(args.train, "enable_gradient_checkpointing", False)
        )
        
        # 兼容新旧格式的 enable_reentrant（旧版如果不存在，通常默认是 True）
        enable_reentrant = (
            gcp_config.enable_reentrant if (gcp_config and hasattr(gcp_config, "enable_reentrant"))
            else getattr(args.train, "enable_reentrant", True)
        )
        # =========================================
        ...
        self.model = build_parallelize_model(
            ...
            enable_gradient_checkpointing=enable_gcp,  # 使用处理好的变量
            basic_modules=list(
                set(getattr(self.model, "_no_split_modules", None) or []) | set(args.model.basic_modules)
            ),
            enable_reentrant=enable_reentrant,          # 使用处理好的变量，不再崩毁
